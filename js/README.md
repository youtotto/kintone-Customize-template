...
| `process-transition-copy.js`    | プロセス遷移後に、条件に応じた計算結果を別フィールドへセット | テータス遷移時のスナップショット保存 / 次工程用の項目コピー     |
| `process-status-field-copy.js` | プロセス管理の現在ステータス・アクション・遷移後ステータスを条件に、同一レコード内のフィールド値を別フィールドへコピー | ステータス遷移時のスナップショット保存 / 次工程用の項目コピー |
...
### 🔹 process-transition-copy.js

プロセス遷移後に、条件に応じた計算結果を別フィールドへセットするテンプレート
...
### 🔹 process-status-field-copy.js

プロセス管理のアクション実行後に、同一レコード内のフィールド値を別フィールドへコピーするテンプレート

#### 📌 使い所
- ステータス遷移時点の値を別フィールドへ保存したい
- 前工程の入力内容を次工程用の項目へコピーしたい
- 申請時点・確認時点・承認時点などのスナップショットを残したい
- 同じ遷移後ステータスでも、アクションごとにコピー内容を変えたい

#### 🧠 特徴
- プロセス遷移専用
- 現在ステータス / アクション / 遷移後ステータスの3条件で制御
- プロセス実行後に、同一レコード内の値を別フィールドへコピー
- sessionStorage を使い、プロセス更新後の detail.show で REST API 更新
- コピー先の上書き可否を設定可能
- コピー元が空の場合のスキップ可否を設定可能

#### ⚠️ 注意点
- ステータス名・アクション名は完全一致で判定されます
- 全角 / 半角 / 前後スペースの違いに注意してください
- コピー元・コピー先はフィールド「名」ではなく、フィールドコードを指定してください
- 添付ファイル、サブテーブルなど一部フィールドは用途に応じて個別調整が必要です
